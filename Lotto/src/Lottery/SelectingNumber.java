...
class NumLblMouseAdapter extends MouseAdapter {
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (idx == SelectingNumber.numOfLottery.length) {
			...
			for (; idx < SelectingNumber.editOfLottery.size(); idx++) {
				if (SelectingNumber.editOfLottery.remove(idx).contains(tempClick))
					;
				break;
...
//	DELETE button MouseAdapter
//class Delete extends MouseAdapter {
//	@Override
//	public void mouseClicked(MouseEvent e) {
...
//		for (; idx < SelectingNumber.editOfLottery.size(); idx++) {
//
//			if (SelectingNumber.editOfLottery.get(idx).contains(click))
//				break;
//		}
//
//		if (SelectingNumber.choiceOfwayList[idx] == null)
//			JOptionPane.showMessageDialog(null, "번호를 먼저 선택해야 삭제가 가능합니다.", "삭제에러", JOptionPane.ERROR_MESSAGE);
//		else {
//			if (idx != SelectingNumber.choiceOfwayList.length - 1) {
//				if (SelectingNumber.choiceOfwayList[idx + 1] != null) {
//					for (int i = idx; i < SelectingNumber.choiceOfwayList.length; i++) {
//						if (i == SelectingNumber.choiceOfwayList.length - 1) {
//							SelectingNumber.choiceOfwayList[i] = null;
//							SelectingNumber.selectedNum[i] = new int[6];
//						} else {
//							SelectingNumber.choiceOfwayList[i] = SelectingNumber.choiceOfwayList[i + 1];
//							SelectingNumber.selectedNum[i] = SelectingNumber.selectedNum[i + 1];
//						}
//					}
//				} else {
		...
		nextLbl.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (choiceOfwayList[select - 1] == null) {
					JOptionPane.showMessageDialog(null, "숫자를 선택하고 넘기세요");
				} else {
					Purchase one = new Purchase(select, selectedNum);
